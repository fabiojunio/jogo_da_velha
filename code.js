...
class TicTacToeGame {
	...
	executeMove(moveIndex){
		if(this.board[moveIndex] == ""){
			...
			if(!this.gameHasWinner()){
				...
				alert("Player " + this.currentPlayer + " é o vencedor!")
				this.start();
			}
	...
	gameHasWinner(){
		...
		return winningCombos.find(combo => {
			if (this.board[combo[0]] != "" && this.board[combo[2]] != "" && this.board[combo[0]] == this.board[combo[1]] && this.board[combo[1]] == this.board[combo[2]]) {
				return true;
			} else {
				return false;
			}
